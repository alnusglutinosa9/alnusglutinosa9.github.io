...
function startAnimation(canvas, coinImage) {
    var wframe = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1000;
    var hframe = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 1000;
    var lineCountFrame = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 6;
    var countSprite = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : 60;
...
var _animateAccordeon = __webpack_require__(5);

__webpack_require__(6);

var _startAnimation = __webpack_require__(2);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

window.$ = __webpack_provided_window_dot_jQuery = _jquery2.default;

var accordeon = __webpack_require__(9);

var _require = __webpack_require__(10),
    startAnimationCanvas = _require.startAnimationCanvas;
...
(0, _jquery2.default)(document).ready(function () {
	...
	setTimeout(function () {
		...
		(0, _animateAccordeon.animateAccordeon)(0);
	}, 700);
...
exports.animateAccordeon = animateAccordeon;
...
function animateAccordeon(i) {
    var duration = 400;

    setTimeout(function () {
        (0, _jquery2.default)("#js-accordion-list .js-accordion-panel").each(function (index) {
            (0, _jquery2.default)(this).delay(duration * index).fadeIn(duration);
        });
    }, 400);

    // console.log($('#js-accordion-list .js-accordion-panel:eq(0)'));
    //
    // $('#js-accordion-list .js-accordion-panel:eq(0)').slideDown(300, function(){
    // 	if (i > 7) {
    // 		return;
    // 	}
    // 	i++;
    // 	$(this).next().slideDown(300, animateAccordeon(i));
    // });
}
...
var _canvasFunction = __webpack_require__(8);
...
var Sprite = function () {
    function Sprite(options) {
        ...
        this.setCanvasSize = _canvasFunction.setCanvasSize.bind(this);
...
exports.setCanvasSize = undefined;
...
function setCanvasSize() {
    ...
    console.log('=================');
    var jsCanvasWrap = document.getElementById('js-canvas-wrap');
    var jsCanvasWrapArray = document.querySelectorAll('.js-canvas-wrap');

    var wrapOffsetWidthCanvas = document.querySelector('canvas').offsetWidth;
    var wrapOffsetHeightCanvas = document.querySelector('canvas').offsetHeight;

    var wrapOffsetWidthSection = document.querySelector('.js-canvas-section').offsetWidth;
    var wrapOffsetHeightSection = document.querySelector('.js-canvas-section').offsetHeight + 150;

    var mobMinWH = Math.min(wrapOffsetWidthSection, wrapOffsetHeightSection);

    var wrapOffsetMinWHSection = Math.min(wrapOffsetWidthSection / 1.7, wrapOffsetHeightSection);
    wrapOffsetMinWHSection = Math.max(wrapOffsetMinWHSection, 320);

    var canwasSizeW = mobMinWH; // wrapOffsetMinWHSection;


    if (wrapOffsetWidthSection < 1024) {
        var canvasFun = document.querySelectorAll('.fun__canvas');

        var canwasWrapSize = Math.min(canwasSizeW, this.wframe);

        if (canvasFun && canvasFun.length) {
            (0, _jquery2.default)(canvasFun).each(function (index, el) {
                el.style.width = canwasWrapSize + 'px'; //Math.min(wrapOffsetWidthSection, wrapOffsetMinWHSection) + 'px';
                el.style.height = canwasWrapSize + 'px'; //Math.min(wrapOffsetWidthSection, wrapOffsetMinWHSection) + 'px';
            });
        } else {

            this.canvas.style.width = this.canvas.style.height = jsCanvasWrap.style.width = jsCanvasWrap.style.height = canwasWrapSize + 'px';
            //this.canvas.style.height = Math.min(wrapOffsetWidthSection, wrapOffsetMinWHSection) + 'px';
        }
        ...
        if (jsCanvasWrapArray && jsCanvasWrapArray.length) {
            (0, _jquery2.default)(jsCanvasWrapArray).each(function (index, el) {
                el.style.width = el.style.height = _this.wWrap + 'px';
            });
        } else {
...
var accordionList = document.getElementById("js-accordion-list");

if (accordionList) {
    accordionList.addEventListener("click", function (el) {
        var panel = el.target.closest('.js-accordion-panel');
        ...
        var panelArray = document.getElementsByClassName("js-accordion-panel");
        $(panelArray).each(function (index, item) {
...
//Accordion
// let acc = document.getElementsByClassName("js-accordion-panel");
// let i;
//
...
exports.startAnimationCanvas = undefined;
...
var _webrtcAdapter = __webpack_require__(11);

var _webrtcAdapter2 = _interopRequireDefault(_webrtcAdapter);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function startAnimationCanvas() {
    var browser = _webrtcAdapter2.default.browserDetails.browser;
    console.log(browser);

    var canvas = document.getElementById('canvas');
    var canvasImg = document.getElementById('img-canvas');
    var canvasFun = document.querySelectorAll('.canvas-fun');
    var canvasJob = document.getElementById('canvas-job');

    if (canvas) {
        var video = document.getElementById("video");

        if (browser === 'chrome' && video) {
            (0, _jquery2.default)('.hero__video').fadeOut;
            (0, _jquery2.default)('.hero__video').show();
            video.play();
            ...
            // let videoAnim = new videoAnimation({
            //     canvas: canvas,
            //     video: video
            // })
        } else {

            (0, _jquery2.default)('.hero__img-canvas').fadeOut;

            var typeWebp = 'image/webp';
            var srcsetWebp = '../img/sprite-min-10-6.webp';
            var srcsetImg = '../img/sprite-min-10-6.png';

            var templateImg = "\n                 <picture>\n                    <source type=" + typeWebp + " srcset=" + srcsetWebp + ">\n                    <img id=\"img-canvas\" src=" + srcsetImg + ">\n                </picture>";

            (0, _jquery2.default)('.hero__img-canvas')[0].insertAdjacentHTML('beforeend', templateImg);

            var coinImage = new Image();
            //coinImage.src = '../img/sprite-min-10-6.webp';

            canvasImg = document.getElementById('img-canvas');

            coinImage.src = canvasImg.currentSrc || canvasImg.src;

            console.log('coinImage.src', coinImage.src);

            coinImage.onload = function () {
                (0, _startAnimation.startAnimation)(canvas, coinImage);
            };
    ...
    if (canvasJob) {
        var _coinImage = new Image();
        //coinImage.src = '../img/sprite-attention-paw.png';

        _coinImage.src = canvasImg.currentSrc || canvasImg.src;
        console.log('coinImage.src', _coinImage.src);

        _coinImage.onload = function () {
            (0, _startAnimation.startAnimation)(canvasJob, _coinImage, 600, 600, 3, 30);
        };

        canvasJob.addEventListener('click', function (e) {
            (0, _startAnimation.startAnimation)(canvasJob, _coinImage, 600, 600, 3, 30);
        });
    }

    if (canvasFun.length) {
        var _coinImage2 = new Image();
        //coinImage.src = '../img/sprite-min-10-6.png';
        _coinImage2.src = canvasImg.currentSrc || canvasImg.src;
        console.log('coinImage.src', _coinImage2.src);
        ...
        // coinImage.onload = function () {
        // 	startAnimation(canvas || canvasFun, coinImage);
        // }

        _coinImage2.onload = function () {
            (0, _jquery2.default)(canvasFun).each(function (index, el) {
                (0, _startAnimation.startAnimation)(el, _coinImage2);
            });
        };
        ...
        (0, _jquery2.default)('.slick-carousel').on('afterChange', function (event, slick, currentSlide, nextSlide) {
        ...
        // On before slide change
        (0, _jquery2.default)('.slick-carousel').on('beforeChange', function (event, slick, currentSlide, nextSlide) {
            (0, _jquery2.default)(canvasFun).each(function (index, el) {
                if (index === currentSlide) {
                    //startAnimation(el, coinImage);
                    // document.querySelector('canvas')
                    // data-slick-index
                    // console.log('getSlick', slick.getSlick().$slides[currentSlide].hidden = true);
                    // console.log('getSlick', slick.getSlick().$slides[nextSlide].hidden = true);

                    //slick.getSlick().$slides[nextSlide].style.opacity = 0;

                    //let canvasFun = document.querySelectorAll('.canvas-fun');
                    var contextFun = canvasFun[nextSlide].getContext('2d');
                    contextFun.clearRect(0, 0, canvasFun[nextSlide].width, canvasFun[nextSlide].height);
                }
            });
        });

        (0, _jquery2.default)('.slick-carousel').slick({
            //infinite: true,
            //adaptiveHeight: true,
            speed: 400,
            //autoplay: true,
...
exports.startAnimationCanvas = startAnimationCanvas;
